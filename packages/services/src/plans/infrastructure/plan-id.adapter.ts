import { planId } from '@wlet/domain/plans'
 ...
 *
 * **A implementação não é escrita aqui, é DELEGADA a `planId`** (§10). A montagem da aplicação
 * tinha um literal próprio — `plan-${Date.now().toString(36)}` — que ignorava o `planId` que
 * `@wlet/domain` já oferecia, e ao reescrevê-lo perdeu o sufixo aleatório. Medido: 200 chamadas
 * no mesmo tique davam 1 id distinto e 199 colisões, contra 200 distintos com `planId`. Dois
