 ...
 *
 * Mora fora do pipeline porque o `scripts/seed.ts` precisa do MESMO casamento: sem ele, o
 * dataset fictício nasce sem `plannedId` nem `receivableId`, e um clone novo abre a tela de
 * Pagamentos dizendo que nove meses de aluguel estão vencidos — num conjunto que paga o
 * aluguel todo mês.
 *
 * É PURO e não toca I/O: nada aqui lê arquivo, descomprime ou calcula id, então este módulo
 * não recebe o `IngestEnv` (`./io.ts`) e continua síncrono nos dois ambientes. O núcleo do
 * ingest é `async` porque precisa esperar o `DecompressionStream` do navegador; uma função que
...
/**
 * O que uma cobrança encontrou no extrato.
 *
 * O original em `scripts/matching.ts` usa este mesmo nome sem declará-lo — `scripts/` roda por
 * tsx, sem typecheck, então o tipo implícito passava despercebido. Aqui ele é declarado com a
 * forma que o próprio código produz, e nada mais: `payers` é `Set` porque quem deve e quem paga
 * nem sempre coincidem, e a tela só nomeia o pagador quando ele não é o devedor.
 */
