...
import { readSheet, serialDate } from './xlsx.ts'
import type { IncomeMonth } from '../src/data/types.ts'
...
export function readBrokerageLedger(dir: string): BrokerageLedger | null {
  const files = readdirSync(dir)
    .filter((f) => f.startsWith('extrato_de_') && f.toLowerCase().endsWith('.xlsx'))
    .sort()
  ...
  for (const file of files) {
    const rows = readSheet(join(dir, file), 1)
  ...
  return { entries, cash: declared ?? cash, source: files, problems }
}
