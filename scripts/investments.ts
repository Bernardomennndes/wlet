...
import { cashAt, contributionsByDate, incomeByMonth, readBrokerageLedger, type BrokerageLedger, type IncomeMonth } from './brokerage.ts'
import { loadCdi, type CdiDay } from './cdi.ts'
import { readSheet } from './xlsx.ts'
import type { InvestmentHolding, InvestmentSnapshot, PatrimonyPoint } from '../src/data/types.ts'
 ...
 *
 * O APORTE não vem daqui nem do banco: vem do extrato da corretora (`brokerage.ts`), que é a
...
function readMovements(path: string): Movement[] {
  const rows = readSheet(path, MOVEMENT_SHEET).slice(1)
...
export function buildInvestments(dir: string): InvestmentReport | null {
  const cdi = loadCdi()
  const files = readdirSync(dir).filter((f) => f.toLowerCase().endsWith('.xlsx'))
  const positionFile = files.find((f) => f.toLowerCase().startsWith('posicao'))
  const movementFile = files.find((f) => f.toLowerCase().startsWith('movimentacao'))
  if (!positionFile || !movementFile) return null

  const problems: string[] = []
  if (!cdi.length) problems.push('CDI não está em cache — rode `pnpm cdi`. Sem ele a renda fixa não tem série histórica.')

  const ledger = readBrokerageLedger(dir)
  ...
  for (const s of POSITION_SHEETS) {
    const rows = readSheet(join(dir, positionFile), s.sheet).slice(1)
    for (const row of rows) {
  ...
  const movements = readMovements(join(dir, movementFile))
